...
describe("<TodoItem />", () => {
  ...
  it("shows line-through on span when done is true", () => {
    ...
    expect(span).toHaveStyle("text-decoration: line-through;"); // DOM에 특정 스타일 적용되어있는지
  });

  // 완료전 line-through not show
  it("does not show line-through on span when done is false", () => {
    const { span } = setup({ todo: { ...sampleTodo, done: false } });
    expect(span).not.toHaveStyle("text-decoration: line-through;"); // not 부정
  });
